...
| suite | คำสั่ง | ผล |
|---|---|---|
| API unit | `cd api && npm test` | **122/122 ผ่าน** |
| API e2e (PostgreSQL 16 จริง) | `cd api && npm run test:e2e` | **92/92 ผ่าน** |
| Web unit | `cd web && npm test` | **33/33 ผ่าน** |
| `tsc --noEmit` | api + web (ทั้ง app และ jest tsconfig) | สะอาดทั้งหมด ไม่มี `any`/`!`/`as` ทับข้อมูลภายนอก |

รวม **247 เทสต์อัตโนมัติ ผ่านทั้งหมด ไม่มีที่ skip** (ตัวเลขนี้อัปเดตหลัง L22/L23 + ค้นหาแผนก — ปิดจบเมื่อ 2026-09-16)
...
### 3a) ยังไม่ได้ implement จริง (9 ข้อ)

| AC | เรื่อง | เหตุผลที่ยังไม่ทำ |
|---|---|---|
| L15 | `sort=department` | จงใจไม่ทำถาวร (ตัดสินใจ 2026-09-16) — แทนที่ด้วยค้นหาแผนกด้วยชื่อบนหน้าจัดการแผนกแทน เพราะในการใช้งานจริงไม่มีใครอยากเห็นแผนกถูกเรียง ดู `PLAN.md §6.1` |
...
> **แก้ไข 2026-09-16** — เพิ่ม L22, L23 (ตรวจข้ามฟิลด์ `salary_min>max`, `join_date_from>to`) พร้อมเทสต์อัตโนมัติครบ (unit ที่ `list-query-validators.spec.ts` + e2e ที่ `employees.e2e-spec.ts`) ย้ายออกจาก 3a แล้ว และตัด L15 ออกจากรายการที่ "รอทำภายหลัง" เปลี่ยนเป็นจงใจไม่ทำถาวรแทน (เหตุผลอยู่แถวเดิมด้านบน) — ตัวเลขสรุปด้านล่างปรับตามแล้ว

**สรุป — 82 ข้อ P1: 64 ข้อมีเทสต์อัตโนมัติยืนยันแล้ว (รวม §5.1 validation P1 ทั้งชุด V02/V03/V06–V18, §7.3 Department CRUD ทั้งหมวด D02–D14 ที่ปิดจบใน S7, และ L22/L23 ที่ปิดจบเมื่อ 2026-09-16), 9 ข้อ implement แล้วแต่ไม่มีเทสต์อัตโนมัติ (3b), 9 ข้อยังไม่ทำจริง (3a)**
...
กับดักเฉพาะของชุดข้อมูลทั้งสี่ข้อผ่านครบ P1 64/82 ข้อมีเทสต์อัตโนมัติยืนยัน, 9 ข้อ implement แล้วแต่ไม่มีเทสต์อัตโนมัติ, เหลือ 9 ข้อยังไม่ทำจริง (บันทึกไว้พร้อมเลข AC ในหัวข้อ 3a/3b) ไม่มีความสามารถนอกขอบเขตหลุดเข้ามา นอกจากนี้มีค้นหาแผนกด้วยชื่อบนหน้าจัดการแผนก (2026-09-16) ที่เป็นความสามารถเพิ่มเติมนอกเกณฑ์ AC ใด ๆ ใน `SPEC.md` — ผู้ใช้ขอเพิ่มตรง ๆ แทนที่ AC-L15 ไม่ใช่การเดา ไม่กระทบหรือขัดกับ AC อื่นที่มีอยู่
